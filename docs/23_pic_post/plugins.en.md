# Plugins

Find the following plugins [here](../../tools/). Use at your own discretion: 

## GIMP 2 (legacy)
GIMP has been upgraded and rebased to GIMP3 making legacy plugins dysfunctional.
If you're still using GIMP 2, you may use the plugin below.
See [official documentation](https://docs.gimp.org/2.10/en/) on the [installation of plug-ins](https://docs.gimp.org/2.10/en/gimp-scripting.html#gimp-plugins-install).

### [Enhance Image](../../tools/GIMP2/plug-ins/enhance_image.py) (legacy)
Creation of image layers to quickly enhance image in post-processing.

## GIMP3
GIMP3 is the latest major version of the __GNU Image Manipulation Program (GIMP)__. 
Be aware that GIMP3 requires folders to be named like the plugin to be installed. 
Find further details in the [official documentation](https://testing.docs.gimp.org/3.2/en/) on [how to install plugins](https://testing.docs.gimp.org/3.2/en/gimp-scripting.html#gimp-plugins-install).

### [Enhance Image](../../tools/GIMP3/plug-ins/python-fu-enhance-image/) (ported)
Direct port from legacy version. To be enhanced in future releases.

All named developments are licensed under the [GNU General Public License v3.0](https://www.gnu.org/licenses/gpl-3.0.html) (GPL-3.0) as included [here](../../LICENSE).